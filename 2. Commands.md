## Table of Contents
- [Quickstart - Just the commands](#Part0.1)    
...
- [Part 4 - Privacy](#Part4)
   - [Install Tor](#Part4.1)
   - [Configure Tor](#Part4.2)
   - [SSH remote access through Tor](#Part4.3)
- [Part 5 - Connecting to Internet](#Part5)
- [Part 6 - RPC (Remote Procedure Calls) API interaction](#Part6)
- [Bonus 1 - Guide: Spend from Multisig Wallet (with Electrum)](#Part_b.1)
---
...
## 📑 Todo List
- Bonus guides
    - [x] Guide: Electrum Server - electrs
    - [ ] Guide: Blockchain explorer - BTC RPC Explorer
    - [ ] Guide: JoinMarket
    - [ ] Guide: BTCPay Server
    - [ ] Guide: Setup Multisig Wallet (with Electrum)
    - [x] Guide: Spend from Multisig Wallet (with Electrum)
...
Plug the external drive and mount it.
   ```js
   sudo apt-get update 
   VERSION=28.1
   ...
   sudo mkdir .bitcoin
   sudo cp bitcoin.conf /home/ubuntu/.bitcoin
   bitcoind --version
   ```
...
5. Update your freshly booted Linux system
   ...
   ```
> [!NOTE]  
> To copy and paste within a terminal on Linux you must use `Ctrl + Shift + C` to copy and `Ctrl + Shift + V` to paste.
...
2. Download from Bitcoin Core repository of Linux binaries, checksums and signatures
   ```js
   wget https://bitcoincore.org/bin/bitcoin-core-$VERSION/bitcoin-$VERSION-x86_64-linux-gnu.tar.gz
   ```
   ```js
...
## Check binaries
   1. Check integrity of image: check that the reference checksum in file `SHA256SUMS` matches the calculated checksum
      ```js
   ...
   2. Check the fingerprint of the public key
      ...
      ```
   3. Signature check: Import the public key and verify the checksums file is cryptographically signed by the signed key
      ```js
      ...
      ```
      If you do not see any good signatures STOP AND DO NOT PROCEED.

## Installation
1. Unpack the Bitcoin Core binaries, install them and check everything is fine by running `bitcoind`. Finally remove the tar file to save disk space
   ```js
   sudo tar -xzvf bitcoin-$VERSION-x86_64-linux-gnu.tar.gz
   sudo install -m 0755 -o root -g root -t /usr/local/bin bitcoin-$VERSION/bin/bitcoin-cli bitcoin-$VERSION/bin/bitcoind
   bitcoind --version
   ...
   rm /home/ubuntu/bitcoin-$VERSION-x86_64-linux-gnu.tar.gz
   ```
   Bitcoin Core now exists within the home directory inside of the `~/bitcoin-$VERSION` folder.
...
2. Check USB3 drive performance. A performant USB3 drive is essential for a node. Let’s check if your drive works well as-is, or if additional configuration is needed.
   1. Install the software to measure the performance of your drive
      ```js
      sudo apt install hdparm
      ```
   2. The external disk should be connected as /dev/sda. Check if this is the case by listing the names of connected block devices
      ```js
      sudo lsblk -pli
      ```  
   3. Measure the speed of the external drive
      ```js
      sudo hdparm -t --direct /dev/sda
      ```        
      Otuput: > Timing O_DIRECT disk reads: 932 MB in  3.00 seconds = 310.23 MB/sec
...
5. Add a symbolic link (soft link) that points the freshly created `.bitcoind` directory on the root to the external hard disk 
   ...
   2. Navigate to the home directory and check the symbolic link is not shown in red (this would indicate an error). The content of this directory will actually be on the external hard disk
      ...
      ls -la .bitcoin
      ```
   3. If the expected output is not obtained, follow the next steps to fix that:
      ...
      2. Create the symbolic link again
         ...
         ```
      3. Check the symbolic link has been created correctly this time, with the expected output:
         ```js
         ls -la .bitcoin
         ```
...
1. Launch the deamon and start up the Initial Block Download (IBD)
    ...
    bitcoind
    ```
    or override the default configuration by specifying the position of the configuration file with an argument:
    ```js
    bitcoind -conf=/home/ubuntu/.bitcoin/bitcoin.conf
    ```

> [!NOTE]  
>* You can always stop at anytime the Initial Blockchain Download (IBD) by pressing on terminal `Ctrl+c`
>* It is important to always properly shut down bitcoin core before turning off the system; this prevents wasted time spent resyncing in the future. To do this navigate into the `~/bitcoin--$VERSION/bin` folder and run `./bitcoin-cli stop`.
...
## #1 Setup login with SSH keys

It’s important to keep your devices safe from brute-force attacks by disabling password login to your server/node altogether and require an SSH certificate. Only someone with physical possession of the private certificate key can log in. With that said we will generate ssh keys on your local machine, also known as ssh-client, and then send them to your ssh-server in this case you Ubuntu server/ node.
...
4. If none of those files exist, or you get a "No such file or directory" error, create a new public / private key pair for the protocol on local machine as a client:
   ```js
   ssh-keygen -t rsa -b 4096
   ```
   When you're prompted to "Enter a file in which to save the key," press Enter to use the default file location.
   Optionally, for maximum security, use `password [A]` to protect your key with a passphrase.
  

6. Copy and install the public ssh key using ssh-copy-id command on a remote server (local IP address).
...
> [!TIP]
> For additional info have a look at
> 
> more: [How To Set up SSH Keys on a Linux / Unix System](https://www.cyberciti.biz/faq/how-to-set-up-ssh-keys-on-linux-unix/?source=post_page-----8b6c6126b223---------------------------------------)
...
If you want to log into your server with SSH when you are away, you can easily do so by adding a Tor hidden service. This makes "calling home" very easy, without the need to configure anything on your Internet router.
...
To make the node reachable from the Internet, it is important to assign a static IP address to forward the ports.
...
4. Save and apply these router settings. Restart the node. It should now get the new static IP address.
...
<a name="part_b.1"></a>
# Bonus 1 - Guide: Spend from Multisig Wallet (with Electrum)

To spend coins from a 2 out of 3 multisig wallet two cosigners / participants need to sign a transaction collaboratively. Any participant can sign the first transaction. Once they sign it, the transaction status will be updated to “partially signed (1/3)” if it is a 2 of 3 wallet. The coins will not be transferred until enough signatures of the multisig wallet has been met.
Familiarize with multisig on testnet before moving on to mainnet.

1. Sign transaction:
   1. Go to "Send" tab
   2. Fill the filed "Pay to" with the recipient Bitcoin address
   3. Input the amount to send
   4. Verify and pay
   
2. Finalize transaction:
   1. Verify the inputs, outputs, receiving address, change address, the fees used and click finalize.
      
      Now the transaction status is changed to "Partially signed (1/3)" and we have a Partially Signed Bitcoin Transaction (PSBT). Do not close the transaction window yet. The other persons has to sign the transaction, afterwards the transaction can be broadcasted to the network. To accomplish this you need to send the PSBT to a second co-signer.
   
3. Export multisig transaction
   There are multiple ways to export transactions. For instance, on the same transaction window towards the left hand side there is "export" option.

   1. Drop down export and there will be shown multiple options to export the transaction.
   2. Copy and send QR code image to the co-signer
   3. Export the file `.psbt` and transfer the file on a USB stick to other computer,  or send the transaction file to other participants in an encrypted message.
   4. Send directly the raw transaction script
  
5. Load PSBT into multisig
   Once the other side receives the transaction details they need to load it into their wallet and sign it.
   1. Open the multisig wallet 2 >> Drop down Tools menu >> Select load transaction >> Choose the import method >> Load transaction
   2. Verify all the information again and sign
   3. Once enough signatures are meet, the broadcast button will become available
   4. Click broadcast to send the signed transaction to network. A message "payment sent" will be displaied
