...
3. Check binary
   i. Check that the reference checksum matches the real checksum
   ```js
...
5. Manually check the fingerprint of the public key:
...
7. Extract the Bitcoin Core binaries, install them and check everything is fine by running `bitcoind`. Finally remove the tar file to save disk space
   ```js
