...
One of the best options to secure the sensitive SSH òogin is to disable passwords altogether and require an SSH certificate. Only someone with physical possession of the private certificate key can log in.
...
2. If files are listed, your public key should be named something like `id_dsa.pub`, `id_ecdsa.pub`, `id_ed25519.pub` or `id_rsa.pub`.
   If one of these files already exists, skip the next step.

3. If none of those files exist, or you get a "No such file or directory" error, create a new public / private key pair:
   ```js
...
4. The public key now needs to be copied to the server.
...
1. Log in to the Raspberry Pi as "admin" using SSH with your SSH key. You shouldn't be prompted for the admin's password anymore.
...
NOTE: Your computer is now running Tor. All that means Tor software is ready to be used by other software. Not all internet traffic is routed over the Tor network, just specific applications that try to connect over Tor (such as Bitcoin Core).   

* **You don't want to run Tor at all:** just remove all Tor configurations
* **You want to run Bitcoin over the regular Interbet _and _ Tor:** Remove all the Tor configurations except for `proxy=127.0.0.1:9050`. This will remove all of the restrictions the other settings make, but still tell Bitcoin Core that you're also running Tor and how to connect over it _as well as_ the regular clearnet way.
