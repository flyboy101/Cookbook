...
6. Navigate to the home directory and check the symbolic link. The content of this directory will actually be on the external hard disk
   ```js
   ls -la .bitcoin
   ```

7. By default `bitcoind` searches for `bitcoin.conf` in the default direcotry `~/.bitcoin`. Download the configuration file (which manages all bitcoind settings) from repo and copy it into the default bitcoin-core's data directory. 
   ```js
...
8. Launching Bitcoin Core deamon and start the Initial Block Download (IBD)
    ```js
    bitcoind
    ```
