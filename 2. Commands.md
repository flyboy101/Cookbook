...
2. Create `tor.list` via nano
   ...
   ```
   where `deb` defines the prcomplied packages hosted in the repository and `deb-src` defines the source code.
   ```js   
...
2. Access with "root"
   ...
   sudo su
   ```
3. Add key GPG used to sign packages
   ```js   
...
6. Check installation
   ...
   tor --version
   ```   
7. Configure Bitcoin Core such that it routes all traffic via TOR. Let allow TOR to accept commands via control port and right authentication. Modify TOR configuration
   ```js
   sudo nano +56 /etc/tor/torrc --linenumbers
   ```   
8. Open port 9051 eliminating symbol `#` from raw Nr. 56
9. Relaunch TOR service to make changes effective 
    ```js
    sudo systemctl reload tor
    ```
10. Check TOR is up and listening default ports 9050 and 9051 in localhost (127.0.0.1)
    ```js
