...
1. Sign transaction:
   Go to "Send" tab, fill the filed "Pay to" with the recipient Bitcoin address and input the amount to send. Verify it and pay.
   
2. Finalize transaction:
   Verify the inputs, outputs, receiving address, change address, the fees used and click finalize. Now the transaction status is changed to "Partially signed (1/3)" and we have a Partially Signed Bitcoin Transaction (PSBT). Do not close the transaction window yet.
   The other persons has to sign the transaction, afterwards the transaction can be broadcasted to the network. To accomplish this you need to send the PSBT to a second co-signer.
   
3. Export multisig transaction
   There are multiple ways to export transactions. For instance, on the same transaction window towards the left hand side there is "export" option. Drop down export and there will be shown multiple options to export the transaction.
   1. Copy and send QR code image to the co-signer
   2. Export the file `.psbt` and transfer the file on a USB stick to other computer,  or send the transaction file to other participants in an encrypted message.
   3. Send directly the raw transaction script
  
4. Load PSBT into multisig
