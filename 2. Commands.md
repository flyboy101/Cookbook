...
1. With user "admin", configure and enable the firewall rules
   ...
   sudo ufw enable
   ```
3. Make sure that the UFW is started automatically on boot
   ```js
   sudo systemctl enable ufw
   ```

4. Check if the UFW is properly configured and active
   ```js
   sudo ufw status
   > Status: active
   >
...
> [!WARNING]  
> If something gets wrong, [Jameson Lopp](https://linode.com/docs/security/using-fail2ban-for-security/](https://blog.lopp.net/how-to-run-bitcoin-as-a-tor-hidden-service-on-ubuntu/) has a Tor tutorial that covers a variety of potential errors at the end of it here. 
