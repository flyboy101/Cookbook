...
4. Check if `ssh` access is allowed (but limited), with:
   ```js
   sudo ufw limit ssh
   ```

5. Add a few more limitations on `ssh`. If you are planning on accessing your node from local network only:
   ```js
   sudo ufw allow from 16.32.64.128 to any port 22
   ```

6. To allow for Bitcoin traffic, do:
   ```js
   # for mainnet ...
