...
9. Download the configuration file from repo and copy it into the default bitcoin-core's data directory. All bitcoind settings are managed by its config file which live under the ~/.bitcoin directory by default
   ```js
...
10. Launching Bitcoin Core deamon and start the Initial Block Download (IBD)
   ...
   You can always kill the client by pressing `Ctl+c`
