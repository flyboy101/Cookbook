...
Bitcoin Core communicate directly with the Tor daemon to route all traffic through the Tor network. We need to enable Tor to accept instructions through its control port, with the proper aothentication.

1. Configure Bitcoin Core such that it routes all traffic via Tor. Let allow Tor to accept commands via control port and right authentication. Modify Tor configuration
   ```js
