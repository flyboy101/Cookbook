...
6. Extract the Bitcoin Core binaries, install them and check everything is fine by running `bitcoind`. Finally remove the tar file to save disk space
   ```js
...
9. By default `bitcoind` searches for `bitcoin.conf` in the default direcotry `~/.bitcoin`. Download the configuration file (which manages all bitcoind settings) from repo and copy it into the default bitcoin-core's data directory. 
   ```js
...
# Part 2: RPC API
1. `bitcoin-cli` is the command-line helper which comes with Bitcoin Core. To check the bitcoin core status run
   ```js
   bitcoin-cli -getinfo
   ```
